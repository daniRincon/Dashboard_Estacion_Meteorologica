export async function requestSerialPort(): Promise<SerialPort | null> {
  try {
    ...
    const port = await (navigator as any).serial.requestPort(); // Pedir puerto
...
export function readSerialStream(port: SerialPort): ReadableStream<string> {
  if (!port.readable) {
...
export function checkDeviceConnection(port: SerialPort): boolean {
...
export async function startSerialCommunication() {
  ...
  if (!checkDeviceConnection(port)) {
    console.error("El dispositivo no está conectado.");
    return;
  }

  console.log("Dispositivo conectado. Iniciando lectura de datos...");
